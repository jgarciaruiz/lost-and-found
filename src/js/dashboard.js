...
var address,
  ...
  function initMap(location) {
      ...
      //console.info('initial zoom: ', map.getZoom());
...
*/
  ...
  function panTo(map, newLat, newLng) {
    ...
    if (currentPan.length > 0) {
      // already doing pan, so queue up positions for next move
      queuedPan.push([newLat, newLng]);
    }
    else {
      ...
      for (var i=0; i < smoothness; i++) {
        currentPan.push([curLat + dLat * i, curLng + dLng * i]);
      }
      currentPan.push([newLat, newLng]);
      setTimeout(function() {
        doPan(map);
      }, 100);
    }
  ...
  function doPan(map) {
    ...
    if (next != null) {
      ...
      //panTo finished but check if there are any queue'd up locations to pan to 
      var queued = queuedPan.shift();
  ...
  function displayMarkers(location, map){
     ...
     var bounds = new google.maps.LatLngBounds();
      ...
      for (i=0; i < storedMarkers.length; i++) {
        ...
        finderuid = storedMarkers[i].finderuid;
        found = storedMarkers[i].found;
        location = storedMarkers[i].location;
        lat = storedMarkers[i].location.lat;
        lng = storedMarkers[i].location.lng;
        status = storedMarkers[i].status;
        ...
        console.info('finderuid: ',finderuid);        
  ...
  function createMarker(map, latlng, found, thumbnail, description, finder){
     ...
     google.maps.event.addListener(marker, 'click', function() {
        ...
        '<div class="iw_title">' + found + '</div>' +
        '<div class="iw_content"><img src="' + thumbnail + '" width="60"><br />' +
        description + '<br />' +
        finder + '</div></div>';
  ...
  var uploader = document.getElementById('thefile');
  ...
  function uploadPic(pic) {
    ...
    uploadTask.on('state_changed', function(snapshot){
      ...
      console.log('Error al subir el pic', error);        
